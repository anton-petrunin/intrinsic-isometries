...
The main result roughly states that the class of these spaces coincides with class of inverse limits of Euclidean $d$-polyhedra. 
\end{abstract}
...
For path isometries, this theorem was proved in \cite[2.4.11]{gromov-PDE},
...
The corollary was proved in \cite{zalgaller} for dimension $\le 4$, 
but a slight modification of the proof works in all dimensions, 
see \cite{krat}.
The 2-dimensional case of this theorem was proved in \cite{krat}.
Later, the proof was extend to all dimensions in \cite{akopjan}; the proof use a \emph{piecewise linear analog of Nash--Kuiper theorem}.
This theorem was proved in \cite{brehm},
but this work was left without attention for many years
and reproved independently in  \cite{akopjan-tarasov}.
...
if it can be presented as an \emph{inverse limit} $\spc{X}=\varprojlim \spc{P}_n$ (see section~\ref{preliminaries}) of a sequence of Euclidean $d$-polyhedra $\spc{P}_n$.
...
the only other example I know is the charcterization of  homogeneous
locally compact metric spaces given in \cite{berestovskii}.
...
\begin{thm}{Exercise}
Show that any compact Riemannian space admits a Lipschitz approximation by Euclidean polyhedra.
\end{thm}
...
Assume $\Omega$ admits an intrinsic\footnote{In fact the same is true for path isometry.} isometry  to $\EE^m$ then $d\le m$ and $\MM^d$ is isometric to a $\EE^d$.
\end{thm}

In particular, the condition \ref{top-dim} on Lebesgue's dimension is not sufficient.

\smallskip

I'm grateful to A.~Akopyan, D.~Burago, S.~Ivanov and anonymous referee for helpful letters and discussions.
...
A \emph{length space} is a metric space such that for any two points $x,x'$ the distance $|x x'|$ coincides with the infimum of lengths of curves connecting $x$ and $x'$.
...
where the infimum is taken along all  $\eps$-chains $(x_i)_{i=0}^n$ from $x$ to $x'$.

Clearly $\dist_{f,\eps}$ is a pre-metric%
...
is well defined.
The pre-metric $\dist_f\:\spc{X}\times\spc{X}\to[0,\infty]$ will be called \emph{pull back metric} for $f$.

A map $f\:\spc{X}\to \spc{Y}$ between length spaces $\spc{X}$ and $\spc{Y}$ is an \emph{intrinsic isometry} if 
$$|xx'|_{\spc{X}}=\dist_f(x,x')$$
...
Then existence of intrinsic isometry $f\:\spc{X}\to \spc{Y}$ implies that $\spc{X}$ is a length space.
\end{thm}

The proof is left to the reader.
Note that for general $\spc{X}$ it is not true.
Consider two points which connected by countable number of unit intervals $\II_n$ 
and one interval of length $\frac12$; 
equip the obtained space with natural intrinsic metric.
Let us remove from our space the interval of length $\frac12$.
The metric on the remaining space $\spc{X}$ is not intrinsic.
...
If $n\ge 2\cdot\pack_\eps \spc{X}$, there are $i$ and $j$ such that $j-i>1$ and $|x_ix_j|\le\eps$.
...
\parit{Proof.}
Assume contrary; that is, there is a sequence of connected subsets $W_n\i \spc{X}$ such that 
...
Assume $(\spc{P}_n)_{n=0}^\infty$ is a sequence of $d$-dimensional Euclidean polyhedra and 
...
Since $|\psi_n(x)\,\psi_n(x')|_{\spc{P}_n}\to |xx'|_{\spc{X}}$ as $n\to\infty$, the map $\imath\:\spc{X}\to\EE^d$ is an intrinsic isometry.\qeds
...
The construction will be done in such a way that if a tiling $\tau'$ is a subdivision of a tiling $\tau$ then for corresponding polyhedra $\spc{P}'$ and $\spc{P}$ there will be a natural intrinsic isometry $\spc{P}'\to \spc{P}$. 
Thus we will construct the needed inverse system of polyhedra out of nested subdivisions of $\EE^d$.
...
glue $\square^{i_1j_1}_n$ to $\square^{i_2j_2}_n$ along $(\imath^{i_2j_2}_n)^{-1}\circ\imath^{i_1j_1}_n$ iff $W^{i_1j_1}_n\cap W^{i_2j_2}\not=\emptyset$.
...
\parit{Proof of proposition \ref{minkowski}.}
...
In this section we will relate the notion of intrinsic isometry defined in section~\ref{preliminaries} with more common (but less natural) notion of path and weak path isometries.
...
one can take a left-invariant sub-Riemannian metric $d$ on Heisenberg group $H$ then factorizing by center gives an weak path isometry $(H,d)\to\EE^2$ (which is not a path isometry and thus not an intrinsic isometry).

\begin{thm}{Example}\label{exam:path} There is a length space $\spc{X}$ and a path isometry $f\:\spc{X}\to \RR$ such that $f^{-1}(0)$ is a  nontrivial connected subset.
...
The following construction was suggested by D.~Burago;
...
\parit{Proof.}
Stat with a metric graph\footnote{That is, locally finite graph with intrinsic metric, such that each edge is isometric to a real interval.} $\Gamma$;
...
Equip $\bar\Gamma$ with the length metric induced by $f$;
that is the diastance between two points $x,y\in \bar\Gamma$ defined as the exact lower bound on the lengths of $f\circ\alpha$ for all paths $\alpha$ from $x$ to $y$.
With this metric, the map $f$ is a path isometry; hence the first part follows.
...
if it can be presented as an inverse limit of Minkowski $d$-polyhedra.
...
This question was asked by D.~Burago; 
...
of Minkowski polyhedra $\spc{M}_n$ with $\dim \spc{M}_n\to\infty$.
\end{thm}
...
\bibitem[Akopyan]{akopjan} Akopyan, A. V.,
...
\bibitem[Akopyan--Tarasov]{akopjan-tarasov} Akopyan, A. V.; Tarasov, A. S.
\textit{A constructive proof of Kirszbraun's theorem.} 
(Russian) Mat. Zametki 84 (2008), no. 5, 781--784. 
\bibitem[Berestovskii]{berestovskii} Berestovskii, V. N.,
   ...
   {Siberian Math. J.},
      ...
      {1},
    {16--25}
\bibitem[BIS]{BIS} Burago, D.; Ivanov, S.; Shoenthal, D.   \textit{Two counterexamples in low dimensional length geometry,}
St.Petersburg Math. J. Vol. 19 (2008), No. 1, Pages 33--43
\bibitem[BBI]{BBI} Burago, D.; Burago, Yu.; Ivanov, S., \textit{A course in metric geometry.} Graduate Studies in Mathematics, 33. American Mathematical Society, Providence, RI, 2001. xiv+415 pp. ISBN: 0-8218-2129-6
\bibitem[Brehm]{brehm} Brehm, U., \textit{Extensions of distance reducing mappings to piecewise congruent mappings on $R\sp{m}$.}  J. Geom.  16  (1981), no. 2, 187--193.
\bibitem[Federer]{ref-to-df} Federer, H., \textit{Geometric Measure Theory,} Springer, 1969.
\bibitem[Knaster]{knaster} Knaster, B. Un continu dont tout sous-continu est ind\'ecomposable. Fundamenta math. 3, 247--286 (1922).
\bibitem[Krat]{krat} Krat, S. \textit{Approximation Problems in Length Geometry,} Thesis, 2005
\bibitem[Lewis]{pseudo.arc}Lewis, Wayne The pseudo-arc. Bol. Soc. Mat. Mexicana (3) 5 (1999), no. 1, 25--77.
...
\bibitem[Gromov]{gromov-PDE}  Gromov, M.,
...
\bibitem[Zalgaller]{zalgaller} Zalgaller, V. A. Isometric imbedding of polyhedra. (Russian)  Dokl. Akad. Nauk SSSR  123  1958 599--601.
\end{thebibliography}
